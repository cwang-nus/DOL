...
def load_adj_mx(adj_filename, node_nums):
    if adj_filename == '':
        adj_mx = np.ones((node_nums, node_nums))
    elif 'pkl' in adj_filename:
        adj_mx = load_pickle(adj_filename)[2]
    else:
        adj_mx = np.load(adj_filename).astype(np.float32)
        num_nodes = adj_mx.shape[0]
        adj_mx = adj_mx + np.eye(num_nodes)
    return adj_mx

def load_supports(adj_filename, node_nums, filter_type='doubletransition'):
    adj_mx = load_adj_mx(adj_filename, node_nums)
    supports = calculate_supports(adj_mx, filter_type)
    return supports

def calculate_supports(adj_mx, filter_type='doubletransition'):
    if filter_type == "scalap":
        supports = [calculate_scaled_laplacian(adj_mx).todense()]
    elif filter_type == "normlap":
        supports = [calculate_normalized_laplacian(
